...
tokenizer = AutoTokenizer.from_pretrained("GujjetiNagaraju/xlm-roberta-base-finetuned-Telugu_NLP")
model = AutoModelForMaskedLM.from_pretrained("GujjetiNagaraju/xlm-roberta-base-finetuned-Telugu_NLP")

## definining where the model weights will be stored
# model_name = "XLMRoberta_Tel"
...
class Xlmr(nn.Module):
    def __init__(self, hf_mod):
        ...
        self.model = hf_mod.roberta
        self.pooler = nn.Linear(in_features=768, out_features=384, bias=True)
        self.dropout = nn.Dropout(p = 0.1, inplace = False)
        self.out_proj = nn.Linear(in_features=384, out_features=3, bias=True)
