...
tokenizer = AutoTokenizer.from_pretrained("Hate-speech-CNERG/deoffxlmr-mono-kannada")
model = AutoModelForSequenceClassification.from_pretrained("Hate-speech-CNERG/deoffxlmr-mono-kannada")

## definining where the model weights will be stored
# model_name = "XLMRoberta_Kan"
...
class Xlmr(nn.Module):
    def __init__(self, hf_mod):
        ...
        self.model = hf_mod
        self.model.classifier.out_proj = nn.Linear(in_features=768, out_features=3, bias=True)  ### we have 3 classes instead of only 2
...
for p in xlmr.model.classifier.parameters():
    p.requires_grad = True

for p in xlmr.model.classifier.out_proj.parameters():
    p.requires_grad = True
